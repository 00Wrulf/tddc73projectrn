  ...
  return (
    <View style={{position: 'absolute', width: widthItem, height: heightItem, backgroundColor: 'rgba(0, 0, 0, 0.4)'}}>
      <ScrollView style={{ width: '89%', backgroundColor: 'white', borderTopLeftRadius: 12, borderTopRightRadius: 12, alignSelf:'flex-end'}}>
        <TouchableOpacity onPress={onClose} style={{ alignSelf: 'flex-start', padding: 10 }}>
          <Text style={{ fontSize: 25, fontWeight: 'bold', color: 'black' }}>&times;</Text>
        </TouchableOpacity>
        ...
        <ScrollView>
          {cartItems.map((prop) => (
            <View key={prop.id} style={{ flexDirection: 'row', alignItems: 'center', padding: 10 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 13, fontWeight: 'bold', width: '70%' }}>{prop.name}</Text>
              </View>
        ...
        <View style={{position: 'fixed', bottom: 0, flexDirection: 'row-reverse', justifyContent: 'space-between', padding: 10, background: 'rgb(255,255,255)', background: 'linear-gradient(0deg, rgba(255,255,255,1) 40%, rgba(255,255,255,0.9) 60%, rgba(255,255,255,0.8393558106836485) 90%, rgba(255,255,255,0.44439782749037116) 100%)' }}>
